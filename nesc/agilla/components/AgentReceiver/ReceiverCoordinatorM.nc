// $Id: ReceiverCoordinatorM.nc,v 1.8 2006/01/13 00:02:44 chien-liang Exp $
...
module ReceiverCoordinatorM {
	...
	uses {
	interface AgentMgrI;
	interface Timer<TMilli> as RecvTimeout0;
	interface Timer<TMilli> as RecvTimeout1;
	interface Timer<TMilli> as RecvTimeout2;
...
implementation {	
	...
	if (call CoordinatorI.stopTimer(id) == SUCCESS) {
		if (call CoordinatorI.startAbortTimer(id) == SUCCESS)				
		return SUCCESS;	
	} 
	return FAIL;
	}
